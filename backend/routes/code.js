...
router.post('/complete', async (req, res) => {
  try {
    ...
    const prompt = `Complete this ${language || 'JavaScript'} code snippet. 
    ...
    const completion = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
      messages: [
        { role: "system", content: "You are a code completion assistant. Provide concise, accurate code completions." },
        { role: "user", content: prompt }
      ],
      max_tokens: 300,
      temperature: 0.2,
...
router.post('/fix', async (req, res) => {
  try {
    ...
    const prompt = `Fix the following ${language || 'JavaScript'} code. 
    ${issue ? `Specific issue: ${issue}` : 'Fix any bugs, syntax errors, or logical issues.'}
    ...
    Provide the fixed code and explanation of changes made.`;

    const completion = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
      messages: [
        { role: "system", content: "You are a debugging expert. Fix code issues and explain your changes clearly." },
        { role: "user", content: prompt }
      ],
      max_tokens: 800,
      temperature: 0.2,
